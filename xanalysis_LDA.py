...
if __name__ == "__main__":
    ...
    #     if therm_mistake is True:
    #         mIc = ds.attrs['mI'] * 0.400342820255
    #     else:
    ...
    #         continue
    #     if not np.isclose(ds.attrs['Delta_P'], dP_rat * mIc, atol=1e-2):
    #         continue
    ...
    print('dP/(mI*c): {0}'.format(dP / (mI * nu)))
    ...
    # # # Seed initial datafile
    # # mE_ds = xr.Dataset({'mE_BECden': mE_da, 'mE_Saddle': SSmE_da}, coords={'aIBi': aIBi_Vals})
    # # mE_ds.to_netcdf(datapath + '/extF_mE.nc')

    # # # Add data to datafile
    # # with xr.open_dataset(datapath + '/extF_mE.nc') as mE_ds:
    # #     mE_ds['mE_homog'] = mE_da
    ...
    #     ax.plot(aIBi_Vals, mE_ds['mE_BECden'].values, 'ro', label='Force + BEC Density')
    #     ax.plot(aIBi_Vals, mE_ds['mE_homog'].values, 'go', label='Force (Homog)')
    #     ax.plot(aIBi_Vals, mE_ds['mE_Saddle'].values, 'bo', label='Analytical Saddle Point')
    #     ax.legend()
    #     ax.set_ylabel(r'$\frac{m^{*}}{m_{I}}$')
    #     ax.set_xlabel(r'$a_{IB}^{-1}$')
    #     ax.set_title('Polaron Mass Enhancement vs. Interaction Strength ($P=0.1$)')

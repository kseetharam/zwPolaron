...
if __name__ == "__main__":
    ...
    outputdatapath = '/home/kis/Dropbox/VariationalResearch/DataAnalysis/newdata/NGridPoints_{:.2E}/steadystate_spherical/aIBi_{:.2f}'.format(NGridPoints_cart, aIBi)
    ...
    # data = np.concatenate((dsa.coords['P'].values[:, np.newaxis], dsa['PB'].values[:, np.newaxis], dsa['NB'].values[:, np.newaxis], dsa['Z_factor'].values[:, np.newaxis], dsa['Energy'].values[:, np.newaxis], dsa['effMass'].values[:, np.newaxis]), axis=1)

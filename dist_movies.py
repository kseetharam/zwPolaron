...
if __name__ == "__main__":
    ...
    # vmin = np.min(nPI_xz_slice_ctVec[-1])
    # vmax = np.max(nPI_xz_slice_ctVec[-1])
    # quad = ax1.pcolormesh(PI_x_g, PI_z_g, nPI_xz_slice_ctVec[1][:-1, :-1], vmin=vmin, vmax=vmax)
    # time_text = ax1.text(0.85, 0.9, 't: {:.1f}'.format(tgrid_coarse[0]), transform=ax1.transAxes, color='r')
    ...
    # anim1 = FuncAnimation(fig1, animate1, interval=50, frames=range(tmax_ind + 1), blit=False)
    # anim1.save(animdatapath + '/nPI_xz.gif', writer='imagemagick')

    # PI_x_g, PI_y_g = np.meshgrid(PI_x, PI_y, indexing='ij')
    # fig2, ax2 = plt.subplots()

    # vmin = np.min(nPI_xy_slice_ctVec[-1])
    # vmax = np.max(nPI_xy_slice_ctVec[-1])
    # quad = ax2.pcolormesh(PI_x_g, PI_y_g, nPI_xy_slice_ctVec[1][:-1, :-1], vmin=vmin, vmax=vmax)
    # time_text = ax2.text(0.85, 0.9, 't: {:.1f}'.format(tgrid_coarse[0]), transform=ax2.transAxes, color='r')
    ...
    # anim2 = FuncAnimation(fig2, animate2, interval=50, frames=range(tmax_ind + 1), blit=False)
    # anim2.save(animdatapath + '/nPI_xy.gif', writer='imagemagick')
    ...
    # vmin = np.min(nPB_xz_slice_ctVec[-1])
    # vmax = np.max(nPB_xz_slice_ctVec[-1])
    # quad = ax3.pcolormesh(PB_x_g, PB_z_g, nPB_xz_slice_ctVec[1][:-1, :-1], vmin=vmin, vmax=vmax)
    # time_text = ax3.text(0.85, 0.9, 't: {:.1f}'.format(tgrid_coarse[0]), transform=ax3.transAxes, color='r')
    ...
    # anim3 = FuncAnimation(fig3, animate3, interval=50, frames=range(tmax_ind + 1), blit=False)
    # anim3.save(animdatapath + '/nPB_xz.gif', writer='imagemagick')

    PB_x_g, PB_y_g = np.meshgrid(PB_x, PB_y, indexing='ij')
    fig4, ax4 = plt.subplots()

    vmin = np.min(nPB_xy_slice_ctVec[-1])
    vmax = np.max(nPB_xy_slice_ctVec[-1])
    quad = ax4.pcolormesh(PB_x_g, PB_y_g, nPB_xy_slice_ctVec[1][:-1, :-1], vmin=vmin, vmax=vmax)
    time_text = ax4.text(0.85, 0.9, 't: {:.1f}'.format(tgrid_coarse[0]), transform=ax4.transAxes, color='r')
    ...
    anim4 = FuncAnimation(fig4, animate4, interval=50, frames=range(tmax_ind + 1), blit=False)
    anim4.save(animdatapath + '/nPB_xy.gif', writer='imagemagick')
    ...
    # fig1, ax1 = plt.subplots()
    # ax1.pcolormesh(PI_x_g, PI_z_g, nPI_xz_slice_st)
    ...
    # fig2, ax2 = plt.subplots()
    # ax2.pcolormesh(PI_x_g, PI_y_g, nPI_xy_slice_st)
    ...
    # fig3, ax3 = plt.subplots()
    # ax3.pcolormesh(PB_x_g, PB_z_g, nPB_xz_slice_st)
    ...
    # fig4, ax4 = plt.subplots()
    # ax4.pcolormesh(PB_x_g, PB_y_g, nPB_xy_slice_st)
    # ax4.set_title('Static Phonon Transverse Momentum Distribution ' + r'($n_{P_{B,x},P_{B,y},st}$)')
    # ax4.set_ylabel(r'$P_{B,y}$')
    # ax4.set_xlabel(r'$P_{B,x}$')
    # ax4.grid(True, linewidth=0.5)

    # fig1.savefig(animdatapath + '/nPI_xz_st.pdf')
    # fig2.savefig(animdatapath + '/nPI_xy_st.pdf')
    # fig3.savefig(animdatapath + '/nPB_xz_st.pdf')
    # fig4.savefig(animdatapath + '/nPB_xy_st.pdf')

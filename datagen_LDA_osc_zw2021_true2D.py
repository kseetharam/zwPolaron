...
if __name__ == "__main__":
    ...
    gaussian_amp_TiSa = 2 * np.pi * A_TiSa_Hz_scaled / T_exp2th  # converts optical potential amplitude to radHz, then converts this frequency to theory units. As we choose hbar = 1 in theory units, this is also units of energy.
    gaussian_width_y_TiSa = wy_TiSa * 1e-6 * L_exp2th
    gaussian_width_x_TiSa = wx_TiSa * 1e-6 * L_exp2th

    gaussian_amp_ODT2 = 2 * np.pi * A_ODT2_Hz / T_exp2th  # converts optical potential amplitude to radHz, then converts this frequency to theory units. As we choose hbar = 1 in theory units, this is also units of energy.
    gaussian_width_x_ODT2 = wx_ODT2 * 1e-6 * L_exp2th
    gaussian_width_z_ODT2 = wz_ODT2 * 1e-6 * L_exp2th
    ...
    for inds in np.arange(Ns):
        x0 = samples[inds, 0]; y0 = samples[inds, 1]; px0 = samples[inds, 2]; py0 = samples[inds, 3]
        trapParams = {'RTF_BEC_X': RTF_BEC_X_th[inda], 'RTF_BEC_Y': RTF_BEC_Y_th[inda], 'omega_BEC_osc': omega_BEC_osc[inda], 'gamma_BEC_osc': gamma_BEC_osc[inda], 'phi_BEC_osc': phi_BEC_osc[inda], 'amp_BEC_osc': amp_BEC_osc[inda], 'omega_Imp_y': omega_Imp_y[inda], 'gaussian_amp_TiSa': gaussian_amp_TiSa[inda], 'gaussian_width_x_TiSa': gaussian_width_x_TiSa, 'gaussian_width_y_TiSa': gaussian_width_y_TiSa, 'gaussian_amp_ODT2': gaussian_amp_ODT2, 'gaussian_width_x_ODT2': gaussian_width_x_ODT2, 'gaussian_width_z_ODT2': gaussian_width_z_ODT2, 'X0': x0, 'Y0': y0, 'PY0': py0, 'PX0': px0, 'L_exp2th': L_exp2th, 'T_exp2th': T_exp2th, 'M_exp2th': M_exp2th, 'omegaX_radHz': omega_x_Na, 'omegaY_radHz': omega_Na[inda], 'omegaZ_radHz': omega_z_Na, 'temperature_K': T, 'zTF_MuM': RTF_BEC_Z[inda]}
    ...
    cParams = {'aIBi': aIBi}
    ds = pf_dynamic_sph.zw2021_quenchDynamics_true2D(cParams, gParams, sParams, trapParams, toggleDict)
    ds.to_netcdf(filepath)
    # ds.to_netcdf('test100')

...
if __name__ == "__main__":
    ...
    ax[0].plot(X_Vals_m * 1e6, n_BEC_Vals * (L_exp2th**3) * 1e-6, 'k-')
    ax[0].set_xlabel('$X$ ($\mu$m)')
    ax[0].set_ylabel('$n(X)$ ($cm^{-3}$)')
    ax[0].set_title('BEC Density Profile')

    ax[1].plot(X_Vals_m * 1e6, EpVals_Hz * 1e-3, 'r-', label=r'$E_{pol}(n(X))$')
    ax[1].plot(X_Vals_m * 1e6, EpVals_harm_Hz * 1e-3, 'b-', label='Harmonic Fit ({:.0f} Hz)'.format(freq_p_Hz))
    ax[1].plot(X_Vals_m * 1e6, VtB_Hz * 1e-3 + np.min(EpVals_Hz * 1e-3), 'g-', label='Shifted BEC Trap ({:.0f} Hz)'.format(freq_tB_Hz))
    ax[1].legend()
    ax[1].set_xlabel('X ($\mu$m)')
    ax[1].set_ylabel('Frequency (kHz)')
    ax[1].set_title('Traps')

    ax[2].plot(X_Vals_m * 1e6, FpVals_interp * 1e24 / F_exp2th, 'r-', label=r'$F_{pol}(n(X))$')
    ax[2].plot(X_Vals_m * 1e6, (-mI * omegap**2 * X_Vals) * 1e24 / F_exp2th, 'b-', label='Harmonic Fit')
    ax[2].legend()
    ax[2].set_xlabel('X ($\mu$m)')
    ax[2].set_ylabel('Force (yN)')
    ax[2].set_title(r'$F_{pol}(n(X))$')

...
for ind, aIBi in enumerate(aIBi_Vals):
    P = 0.85
    DP, Nph, Nph_x, nPB_Tot, nPB_Mom1, beta2_kz_Mom1, x, y, z, nx_x_norm, nx_y_norm, nx_z_norm, kx, ky, kz, nPB_kx, nPB_ky, nPB_kz = np.loadtxt(datapath + '/3Ddist_aIBi_{:.2f}_P_{:.2f}.dat'.format(aIBi, P), unpack=True)
...
for ind, P in enumerate(PVals):
    aIBi = -2
    DP, Nph, Nph_x, nPB_Tot, nPB_Mom1, beta2_kz_Mom1, x, y, z, nx_x_norm, nx_y_norm, nx_z_norm, kx, ky, kz, nPB_kx, nPB_ky, nPB_kz = np.loadtxt(datapath + '/3Ddist_aIBi_{:.2f}_P_{:.2f}.dat'.format(aIBi, P), unpack=True)
    Nph_Vec_P[ind] = Nph[0]; NphX_Vec_P[ind] = Nph_x[0]; nPBM1_Vec_P[ind] = nPB_Mom1[0]; b2M1_Vec_P[ind] = beta2_kz_Mom1[0]
...
axD[2, 0].plot(np.array(PVals), Nph_Vec_P, label=r'$|\beta_{\vec{k}}|^2$', color=colorList[0], linestyle='-')
axD[2, 0].plot(np.array(PVals), NphX_Vec_P, label=r'$n(\vec{x})$', color=colorList[1], linestyle='-')
axD[2, 1].plot(np.array(PVals), b2M1_Vec_P, label=r'$|\beta_{\vec{k}}|^2$', color=colorList[0], linestyle='-')
axD[2, 1].plot(np.array(PVals), nPBM1_Vec_P, label=r'$n_{\vec{P_B}}$', color=colorList[1], linestyle='-')
